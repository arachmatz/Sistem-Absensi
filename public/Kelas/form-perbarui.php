    ...
    <div class="col-xs-10">
      <div class="box">
        ...
        <?php 
            ...
            $Kelas->select($Kelas->getTable())->where()->comparing("id_Kelas",$id_Kelas)->ready();

            $row = $Kelas->getStatement()->fetch();
        ...
        <form action="<?php url("updateKelas/") ?>" method="post">
            <div class="box-body">
                <input type="text" value="<?php echo $id_Kelas ?>" class="hide" name="id_Kelas">
                <div class="form-group">
                    <label for="KelasInput">Id Kelas</label>
                    <input type="text" class="form-control" id="KelasInput" placeholder="Masukan id" name="id_kelas" value="<?php echo $row['id_kelas'] ?>" readonly>
                </div>
                <div class="form-group">
                    <label for="KelasInput">Nama Kelas</label>
                    <input type="text" class="form-control" id="KelasInput" placeholder="Masukan nama kelas" name="nama_kelas" value="<?php echo $row['nama_kelas'] ?>">
                </div>
                <div class="form-group">
                    ...
                    if($guru->getStatement()->rowCount()){
                      while($rowGuru = $guru->getStatement()->Fetch()){ ?>
                        <option value="<?php echo $rowGuru['nip'] ?>" <?php if($row['wali_kelas'] == $rowGuru['nip']){ ?> selected <?php } ?>><?php echo $rowGuru['nip'].'-'.$rowGuru['nama_guru']; ?></option>
                      <?php
                      }
                    }
                    ?>
