        ...
        ?>
          <div class="row">
            <div class="col-md-8">
              ...
              <h4>Jadwal mengajar anda pada hari <br> <?php echo $hariSeleksi.', '.date('d F Y') ?></h4>
              ...
              if($jadwal->getStatement()->rowCount()){
                ...
                <tr>
                  ...
                  <td><?php echo $rowJadwal['id_kelas_jadwal'] ?></td>
                </tr>
              <?php 
                }
              }else{ ?>
              ...
              ?>
                ...
                <form action="<?php url('addAbsensi/') ?>" method="post">  
                  <div class="form-group">
                    <label for="nipInput">no induk</label>
                    <input type="text" class="form-control" name="nip" value="<?php echo $_SESSION['id_user'] ?>" readonly>
                  </div>
                  <div class="form-group">
                    <label for="nipInput">Hari</label>
                    <input type="text" class="form-control" name="hari" value="<?php echo $hariSeleksi ?>" readonly>
                  </div> 
                  <div class="form-group">
                  ...
                  <!--<div class="form-group">
                    <label for="nipInput">Keterangan </label><br>
                    <input type="radio" name="keterangan" value="H"> Hadir &nbsp;
                    <input type="radio" name="keterangan" value="I"> Izin &nbsp;
                    <input type="radio" name="keterangan" value="S"> Sakit &nbsp;
                    <input type="radio" name="keterangan" value="A"> Tanpa Keterangan &nbsp;
                  </div> -->


                  <input type="submit" class="btn btn-primary" name="mulai" value="mulai Absen">
                </form>
              </div>
            </div>
          </div>
              <?php
              // jadwal berdasakan guru, tanggal dan jam setelah absen dibuat
              $jadwal->select($jadwal->getTable())
              ->join('mata_pelajaran','jadwal.kode_mapel_jadwal','mata_pelajaran.kode_mapel')
              ->join('guru','jadwal.nip_jadwal','guru.nip')
              ->where()->comparing('nip',$nip)
              ...
              $absensi->select($absensi->getTable())->where()->comparing("id_jadwal_absensi",$rowJadwal['id_jadwal'])->ready();
              ...
              <table class="table table-bordered">
                ...
                <?php 
                  $absensi = new \model\Absensi();
                  $absensi->select($absensi->getTable())->where()->comparing("id_jadwal_absensi",$rowJadwal['id_jadwal'])
                  ->ready();
                  ...
                  while ($rowAbsen = $absensi->getStatement()->fetch()) {
                    ...
                    if($tanggal[0] == $tanggalReq && $jam == $rowJadwal['jam']){ ?>
                      <tr><td><?php echo $rowAbsen['id_absensi']; ?></td>  
                        ...
                        <form action="<?php url('absenIzin/') ?>" method="post">
                        <input type="text" name="id_absensi" value="<?php echo $rowAbsen['id_absensi'] ?>" class="hide">
                        <input type="text" name="nis" value="<?php echo $rowAbsen['nis_absensi'] ?>" class="hide">
                        <button class="btn btn-warning" name="absen">I</button>
                        </form>&nbsp;
                        <form action="<?php url('absenSakit/') ?>" method="post">
                        <input type="text" name="id_absensi" value="<?php echo $rowAbsen['id_absensi'] ?>" class="hide">
                        <input type="text" name="nis" value="<?php echo $rowAbsen['nis_absensi'] ?>" class="hide">
                        <button class="btn btn-warning" name="absen">S</button>
                        </form>&nbsp;
                        <form action="<?php url('absenAlpha/') ?>" method="post">
                        <input type="text" name="id_absensi" value="<?php echo $rowAbsen['id_absensi'] ?>" class="hide">
                        <input type="text" name="nis" value="<?php echo $rowAbsen['nis_absensi'] ?>" class="hide">
                        <button class="btn btn-danger" name="absen">A</button>
                        </form>&nbsp;
                        </td>
                      </tr>
                    <?php 
